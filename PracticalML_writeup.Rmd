...
rf <- train(dt.train$classe ~ ., data=dt.train, method = "rf", trainControl=tc)
```

```{r}
fit.rpart <- train(classe ~ ., data=dt.train, method = "rf")
```

```{r}
pred.rf <- predict(fit.rf, dt.cv)
confusionMatrix(pred.rf, dt.cv$classe)
```

```{r}
pred.rpart <- predict(fit.rpart, dt.cv)
confusionMatrix(pred.rpart, dt.cv$classe)
```
